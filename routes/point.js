...
router.get('/orders/:id', async (req, res) => {
  try {
    ...
    res.json(pointOrdersStore.getOrder(req.params.id));
  } catch (error) {
...
// Solo disponible fuera de producción: fuerza el estado final de la orden
// usando el endpoint de simulación de Mercado Pago (o el mock local), para
// poder ver la demo completa sin una terminal física.
router.post('/orders/:id/simulate', async (req, res) => {
  if (process.env.NODE_ENV === 'production') {
    return res.status(403).json({ error: 'La simulación solo está disponible en entornos de desarrollo/pruebas' });
  }
